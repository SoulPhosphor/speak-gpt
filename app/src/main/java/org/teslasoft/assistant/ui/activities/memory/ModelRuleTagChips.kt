 ...
 * searches the model-rule tag pool in a dropdown; picking a suggestion or
 * confirming the text (IME done) attaches the tag, creating it if it's new.
 * Selected tags render as removable chips. This is a SEPARATE pool from the
 * roleplay tag realm and from memory tags — it touches only
 * model_rule_tags. The controller only edits its in-memory selection; the
 * hosting editor persists the links on save.
 */
class ModelRuleTagChips(
    ...
    private val selected = LinkedHashMap<String, ModelRuleTagRecord>()

    /** The whole pool, loaded once off-thread; refreshed after a create. */
    private val pool = ArrayList<ModelRuleTagRecord>()
        ...
        Thread {
            try {
                if (MemoryStore.isProvisioned(activity)) {
                    val all = MemoryStore.getInstance(activity).getModelRuleTags()
    ...
    fun setInitial(tags: List<ModelRuleTagRecord>) {
        selected.clear()
    ...
    private fun addTag(tag: ModelRuleTagRecord) {
    ...
    private inner class FuzzyTagAdapter :
        ...
        fun resultAt(position: Int): ModelRuleTagRecord? = results.getOrNull(position)
        ...
        override fun getFilter(): Filter = object : Filter() {
            ...
            override fun publishResults(constraint: CharSequence?, filterResults: FilterResults?) {
                results = (filterResults?.values as? List<ModelRuleTagRecord>) ?: emptyList()
                clear()
