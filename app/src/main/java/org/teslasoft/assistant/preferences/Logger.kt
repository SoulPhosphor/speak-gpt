...
class Logger {
    companion object {
        ...
        fun log(context: Context, type: String, tag: String, level: String, message: String) {
            ...
            // Local time, second precision, 12-hour clock. The ISO/UTC instant
            // this used to print ("2026-06-12T19:03:31.903759Z") was unreadable
            // on a phone and in the wrong timezone, which made correlating a
            // log line with "the turn that just failed" impossible. The later
            // 24-hour local format (owner request, July 20 2026) was replaced
            // with 12-hour + AM/PM for the same reason.
        ...
        // A new entry begins at a line starting with the "[yyyy-MM-dd h:mm:ss a] "
        // header Logger writes; everything up to the next such line (e.g. a
        // multi-line stack trace) belongs to the same entry. The AM/PM suffix is
        // optional in the regex (but not in LOG_TIME_FORMAT) so entries already on
        // a device from before the July 20 2026 12-hour switch still match as
        // header lines and split correctly; such an old-format timestamp still
        // fails to parse against LOG_TIME_FORMAT below and is handled by the
         ...
         * the "[yyyy-MM-dd h:mm:ss a] " entry header and split this record when
