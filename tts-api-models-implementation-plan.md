...
Persist the immediately previous selected voice separately from the current selection, including enough source identity to restore it correctly. Record this when the user activates a different voice, not when they browse or preview. The existing LastKnownGoodVoiceRegistry stores one selection, so do not assume it already provides previous-selection history or silently change its existing Google behavior. Missing history on older preferences is handled by the explicit dialog in Section 7.7, not by inventing a prior selection.
...
8. Activate a voice with enough identity to resolve its saved source and exact voice. Preserve the immediately previous user-selected voice for Section 7.7 recovery. Retain existing preference scope; browsing and preview do not change that history.
