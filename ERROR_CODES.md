...
defined as everything from one `[yyyy-MM-dd h:mm:ss a] …` header line up to — but
...
already prefixes every line with `[yyyy-MM-dd h:mm:ss a] [GenError] [ERROR]`, so
...
- **Retention (§4.2).** Replace `Logger`'s single character-count trim
  ...
  **Trim whole entries, not physical lines.** An entry spans from one
  `[yyyy-MM-dd h:mm:ss a] …` header line up to the next header line, so a
