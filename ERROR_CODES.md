...
| `[S4]` | Generated output stopped by content filter | Output-side structured evidence or output terminal finish reason | `[S4] The provider stopped the model-generated output because of a content filter. This does not mean the user's request was rejected.` | Exact provider response, filter side, partial-response metadata. |
| `[S5]` | Content filter fired; side not reported | Structured filter evidence without input/output attribution | `[S5] A provider content filter stopped the generation, but the provider did not report whether it filtered the input or generated output.` | Exact provider response and `ambiguous` filter side. |
