...
| `[N2]` | Connect timeout — could not establish the connection in time | `"Connect timeout has expired"` or `ConnectTimeoutException` | `[N2] Connection Timed Out: The app could not establish a connection to the server within the connection timeout. This usually points to a slow or unstable network rather than the model. Try again, or raise the connection timeout for this endpoint.` | Nothing extra; standard block only. No stack trace. The per-endpoint Connection Timeout is the value to raise. |
...
| `[N4]` | Response (read/socket) timeout — connected, but no reply arrived in time | `"Socket timeout has expired"` or `SocketTimeoutException` / `HttpRequestTimeoutException` | `[N4] Response Timed Out: The connection succeeded, but the server did not send a response within the response time. The model may be busy or slow to reply. Try again, switch models, or raise the response time for this endpoint.` | Nothing extra; standard block only. No stack trace. The per-endpoint Response Time is the value to raise. `[N2]` (connect) and `[N4]` (read) split what was one timeout code, so the message names which timer fired. |
| `[C1]` | Client/app cancellation wrapped by another exception | A `CancellationException` in the cause chain | `[C1] The client canceled the request before it completed.` | Cancellation cause; never attributed to a provider. |
...
| `[A2]` | Authenticated request forbidden | Outer HTTP 403 without content-filter evidence | `[A2] The service refused access to this request. Check the account and model permissions.` | Exact provider response and separate outer/embedded statuses. |
...
| `[M4]` | Invalid or unsupported request parameter | Structured provider type/code such as `invalid_request_error` or `unsupported_parameter` | `[M4] The provider rejected an unsupported or invalid request parameter. The provider's exact explanation is shown below.` | Exact provider response and structured code/type. |
| `[M5]` | Provider refused the request; no specific part identified | Outer or embedded client-error status (4xx) after auth, payment, not-found, payload-size and rate-limit codes have been matched | `[M5] The provider rejected the request and generated nothing. It did not say which part it objected to — its exact wording is below.` | Exact provider response, separate outer/embedded statuses, and the outbound request field names. |
...
| `[S2]` | Response could not be read as the expected type — no converter for the body (`NoTransformationFoundException`, often a non-streaming error body, usually HTTP 400), or the body was parsed but was not a valid completion (`JsonConvertException` wrapping kotlinx's `MissingFieldException`, e.g. an endpoint that answers with an error envelope or an empty body) | `"NoTransformationFoundException"`, `"Expected response body of the type"`, `"JsonConvertException"`, `"MissingFieldException"`, or `"required for type with serial name"` | `[S2] The server returned a response the app could not read as a stream. Check whether this endpoint and model support streaming.` | **Full stack trace** plus **HTTP status if available** — this case is ambiguous and the trace is often the only place the underlying HTTP error survives. |
| `[S3]` | Prompt rejected as inappropriate content | `"Your request was rejected"` | `[S3] Your message was blocked by the provider's content filter. The model never received it, and nothing was generated. You can edit the message and send it again.` | Nothing extra. Do **not** log the prompt text. |
| `[S4]` | Generated output stopped by content filter | Output-side structured evidence or output terminal finish reason | `[S4] Your message went through. The provider's content filter stopped the model partway through writing its reply, so the answer is cut short. Nothing you sent was blocked.` | Exact provider response, filter side, partial-response metadata. |
| `[S5]` | Content filter fired; side not reported | Structured filter evidence without input/output attribution | `[S5] A content filter stopped this exchange, but the provider did not say whether it blocked your message or the model's reply. The provider's own words are below.` | Exact provider response and `ambiguous` filter side. |
| `[S6]` | Provider/upstream/routing failure | Structured provider type or outer/embedded 5xx | `[S6] The provider, upstream model service, or routing service failed while handling the request.` | Exact provider response plus separate outer and embedded statuses. |
...
- **Catch-all (`U0`).** Any exception not matched by a known condition maps to
  ...
  neutral `Unclassified generation failure` heading, exception class, message,
  and full stack trace. The Error Log never tells the reader that details were
  saved to the Error Log. No raw trace ever reaches the chat.
