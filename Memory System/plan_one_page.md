...
The long counterplan files are agent machinery. The owner does not need to read them. This page records the current product decisions; detailed binding computer-workflow wording also lives in `computer_memory_review_rulings.md`.

*(Status recorded 2026-07-30. Agents: keep this page true whenever decisions change.)*
...
The audit uses the same durable foreground-service pattern as API Memory Assistant. The user may move between screens, continue chatting, or turn the screen off. A durable run recovers after process interruption. Force-stopping ends the live service.
...
Show export progress, completion, and errors beneath this section.
...
If some proposals are invalid, import the valid proposals and show how many could not be imported.
...
The major structure and copy decisions are complete. Later visual review may refine minor success, empty-result, package-export completion, and failure wording after the first implementation is visible. Do not reopen the architecture or ask the owner to repeat the decisions above.
...
- implement and test durable progress, restart recovery, malformed files, partial imports, duplicate prevention, and stale-record conflicts;
- reconcile the long counterplan against these binding rulings before merge.

No app code has been changed by these planning commits.