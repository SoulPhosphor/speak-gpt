...
This focused specification accompanies `external_memory_analysis_counterplan.md` and `memory_controls_and_pending_ui_copy.md`. It supplies the exact user-facing wording and behavior contract for live memory retrieval controls and analysis chunk choices. Where these controls are implemented, use this wording rather than asking the owner to decide it again.

## 1. Memory Retrieval

**Section Title:** `Memory Retrieval`

These controls limit only how much approved memory may be included in one live AI request. They do not limit how many memories may be stored on the device.
...
The app stops adding retrieved memory when either the selected memory-count maximum or selected memory-token maximum is reached, whichever occurs first.

This setting does not limit stored memory on the device.
...
- `Balanced` considers both enabled pools without allowing one pool to consume all limited context before the other is considered;
- `General Memories First` prefers General memories when relevant results compete for limited context;
- `Companion Memories First` prefers the current companion's memories when relevant results compete for limited context;
- priority never makes an irrelevant memory eligible;
- unused capacity from one pool may be used by another enabled pool.
...
1. determine which memory pools the conversation permits;
2. search only eligible General, Companion, and Roleplay scope/target contexts;
3. reject memories below the selected Match Strictness;
4. rank primarily by semantic relevance;
5. apply optional importance only as the approved secondary signal;
6. apply Memory Priority when relevant General and Companion results compete;
7. add memories until the count or token maximum is reached;
8. when Use Model-Aware Limits is On, reduce only from verified or manual context information;
9. keep fixed app safety, developer instructions, and fixed companion identity above retrieved memory context;
10. insert retrieved memory before the conversation transcript.
