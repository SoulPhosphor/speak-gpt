# External Memory Analysis + RAG Compatibility — Counter Plan (Revision 11, numbered phases)

> **This is the only active memory-system delivery plan.** Current code and
> focused tests are evidence of what exists; neither commit messages nor a
> second planning document may declare a feature complete. Files elsewhere in
> `Memory System/` are narrow technical references only. If a reference
> conflicts with this plan or the owner's current direction, this plan and the
> owner's direction win.
>
> **Attribution warning:** older files—and older passages retained in this
> file—use labels such as “owner-approved,” “RULED,” and “said in chat.” The
> available record does not authenticate those labels. They are historical
> requirement claims, not approval. Treat every such behavior, UI, and wording
> choice as provisional until the owner directly confirms it in the work unit
> that needs it.

**Document type:** Planning/architecture only — no code, database, prompt, UI,
or string changes were made with this document.
**Original baseline:** verified against the then-current repository and remote branch state
...
**Revision 10 verification baseline:** `main` at `91b1e79`, 2026-08-03.
...
revision.
**Revision 10 (2026-08-03):** restored this file as the single active plan
after a later roadmap rewrite incorrectly superseded it, moved it to
`legacy/`, and promoted unverified completion claims. This revision adds a
code-backed status audit against `main` at `91b1e79`, records the exact later
UI decisions that can be traced in Git history without calling them verified
owner quotes, and replaces §15 with a one-work-unit-at-a-time repair order.
No app behavior is changed by this revision.
**Revision 11 (2026-08-03):** finishes the reorganization that Revision 10
left incomplete. Revision 10 split only Stage 1 into bounded work units, then
left Phases 2–4 as three end-to-end bundles. Section 15 now gives every
required feature in Phases 1–4 its own dependency-ordered work unit, makes all
of Stage 1 a hard prerequisite for Phase 2, and places the shared filing,
acceptance, evidence, and mutation boundaries before any screen or transport
that consumes them. It also incorporates the later owner direction for
per-capture companion Memory Participation and Archive behavior, removes the
obsolete Include Earlier Messages dialog/question from the active plan, and
forbids a new withheld-items UI. No app behavior is changed by this revision.

> **Approval note:** nothing in this document—or in the historical files named
> “owner approved”—is approved wording or an approved screen merely because a
> document says so. Every user-facing decision goes through the owner directly
> before it is built, per the OWNER APPROVAL GATE in `CLAUDE.md`. A document is
> evidence of a prior claim, never proof of approval.

---

## Current reality — verified against `main` at `91b1e79` (2026-08-03)

### Status words have strict meanings

- **Code present:** named production code and a reachable entry point exist.
- **Automated:** focused tests cover the requirement, not merely a parser or a
  helper in isolation.
- **Device-verified:** the complete user path was exercised on an Android
  device from entry point through stored result.
- **Complete:** code present + automated + device-verified, with no known gap
  against this plan. A green compile alone is not complete.

No step below is called complete merely because a commit message said
"shipped."

| Step | Verified state on `main` | Known gap before Complete |
|---|---|---|
| **1.1 Source and participation semantics** | **Partial on audited `main`.** The four-option Memory Engine picker exists, and Quick Settings has independent Associative Memory and Lorebook switches. The unmerged `claude/feature-1a-archive-chat-ytc4o5` branch through `aa90bd9` adds per-capture companion participation, archive-pause transitions, and focused transition tests. | The branch is not part of the audited `main`, is not device-verified, and its General path still needs the complete filing/prompt filter. Work Unit 1 must verify the settled Full / General / None ceiling, Archive precedence, per-capture behavior, and no-new-UI rule end to end before this step can be Complete. |
| **1.2 Retrieval correctness** | **Code present + focused JVM tests.** Complete-set lexical fallback, relevance-before-top-K, backfill, policy clamps, current freshness, target aliases, and `memory-doc-v2` exist. | Run the focused suite on the repair branch and one device retrieval check before marking Complete. |
| **1.3 Run integrity and truthful status** | **Code present.** Sealed transcript claims, durable run rows, startup reconciliation, the foreground service, scene-context capture, rename-safe rejection keys, and type-aware results are wired. | There is no focused automated test covering mid-run append, process death, claim release, and exactly-one live run as one lifecycle. Add it, then device-test leaving the screen and killing/restarting the process. |
| **1.4 Index lifecycle and performance** | **Code present + partial focused tests.** Scoped vector loads, complete-set fallback on a partial index, single-flight missing-vector repair, lore/comparison vector caches, and debug notes exist. | Add a repair-lifecycle test proving one repair job, retry after failure, correct invalidation, and persistent/inspectable failure diagnostics. Device-test a deliberately missing vector. |
| **1.5 Provenance and hygiene** | **Partial and unsafe to call complete.** Placement-aware deterministic matching, embedding-assisted memory-to-memory candidates, Pending cards, and a comparison screen exist. | Main has no shared filing/acceptance boundary, no transcript-row/excerpt evidence lineage, no transaction-time exact recheck for direct Save/editor acceptance, no Lorebook/card comparison candidates, and target resolution still silently takes the first duplicate name. The comparison screen also implements later action semantics that conflict with this plan: **Save & Replace permanently deletes old memories**, and a conflicted suggestion has no **Keep Existing/Delete Suggestion** resolution. |
| **1.6 Lore query and diagnostics** | **Code present + focused JVM tests.** Batched lore queries, exact cross-book dedup, budget selection, and matched/injected/cut logging are wired into chat assembly and debug. | Run the focused suite and a device turn with two books, duplicate content, and a forced budget cut before marking Complete. |
| **1.7 Model-free analysis and Lorebook Memories** | **Partial.** The two-type analysis picker, type-specific prompt/parser/run history, Lorebook suggestion tables, result link, Lorebooks/Pending split, destination selection, full-page new-book flow, edit/approve/delete actions, and parser/failure tests exist. | No end-to-end storage/navigation/approval tests exist. Approval writes the Lorebook entry and consumes the pending suggestion in separate databases without an idempotent commit ledger, so interruption can leave a duplicate-able pending item. The destination control is styled like a dropdown but opens a separate list dialog. The missing-model UI follows a later dismissible-dialog record, while this counterplan specifies a persistent inline banner. Resolve that single visible conflict only when its work unit begins. |

### Later UI records found in Git history

These details were written after Revision 9 and much of the current code
follows them. The available conversation search could not authenticate them as
the owner's exact statements, so they are preserved here as **traceable later
records**, not quoted owner law:

- `563bbc7` and `920f7a2`: Lorebook Suggestions live in the Lorebooks area;
  the Lorebooks/Pending split exists only while suggestions are pending; cards
  show entry text and trigger keywords with no separate title; destination can
  choose an existing Lorebook or the normal full-page new-Lorebook flow; Edit,
  Approve, and Delete are available.
- `38e4c05`, `813d7a4`, `7bec30a`, and `d34bfde`: Memory Analysis Type became
  a two-option **Associative Memories / Lorebook Memories** dropdown in a
  two-column row with three visibly separated helper paragraphs. Current code
  matches this shape.
- `c179749`, `b51c169`, `fb057b9`, and `e0cca37`: a later record replaced the
  counterplan's persistent missing-model banner with a dismissible per-visit
  reminder. Current code follows the later record. This remains an explicit
  conflict until the owner identifies which behavior is real.
- `7ef6ecf`, `954ba9e`, and `b0f253b`: a later Possible Match design replaced
  the counterplan's Keep Both / Keep Existing / Replace Existing / Delete
  Suggestion set with Save & Edit Old Memory / Save & Supersede / Save &
  Replace. Current code follows the later design and makes Replace a permanent
  delete. Because that changes data destructively and the later record could
  not be authenticated, §15 repairs this before expanding the feature.
- `7fc0133`, `82cbbee`, and `aa90bd9`: the unmerged Stage 1.1 branch captures
  archive-paused and companion-opt-out material with separate semantics and
  adds a companion participation selector. These commits are implementation
  evidence, not completion. The later owner ruling in Revision 11 controls:
  participation is captured with the transcript, later setting changes are
  not retroactive, Archive cannot raise a row above its companion ceiling,
  and this work adds no withheld-items or second Pending surface.

### Execution discipline

Exactly one §15 work unit may be active. Each unit gets its own feature branch
and contains only the code, tests, and directly affected documentation for that
unit. It does not add placeholder rows, future screens, or refactors for a
later unit. The next unit does not start until the current unit has focused
tests, green Android CI, a written on-device check, and an evidence-backed
status update in this file.

If visible behavior has conflicting records, build and test every invisible
part first, then stop at one clearly stated decision. Never turn one conflict
...
- **(f) Separate source injection, companion participation, and archive
  consent.** The current owner rule is that prompt sources, what a companion
  may contribute to long-term memory, and whether a chat is currently paused
  from review are separate decisions. Therefore:
  ...
  - **Use lore books in this chat** controls classic trigger-based lorebook
    injection only.
  - The companion's existing **Memory Participation** control sets the
    maximum allowed participation captured with each transcript row: Full
    permits normal outputs; General permits general/model-rule/Lorebook work
    but filters companion-specific relationship memory; None excludes the
    captured row from Archivist, Associative, and Lorebook analysis.
  - **Archive this chat** pauses or resumes review eligibility, but it cannot
    raise a row above the participation ceiling captured when that row was
    created.
  Turning either source off must not mark transcript rows excluded, and
  turning it back on must not alter the review queue. A participation change
  applies only to newly captured material; it never reclassifies older rows.
  Turning Archive back on clears only the archive-pause reason from otherwise
  unprocessed rows. Rows captured under None remain excluded, while Full and
  General retain their original ceilings. Preserve older exclusions that
  cannot be attributed safely. Do not add an Include Earlier Messages dialog,
  a withheld-items list, a second Pending destination, or another control for
  this behavior. This must land before Phase 2 because export eligibility
  depends on it.
...
### 5.4 Prompt sources, participation, and archive state are independent

The old capture path used a prompt-injection preference as an exclusion cause.
That coupled three separate questions: what enters the current prompt, what a
particular companion may contribute to long-term memory, and whether this chat
is paused from later review. The settled contract separates them.

| Control | Prompt effect | Review/capture effect |
|---|---|---|
| Saved memories | RAG/scene/card retrieval only | None |
| Lore books | Classic trigger-based lore only | None |
| Companion Memory Participation | Full permits normal companion + general memory use; General excludes companion-relationship memory while preserving general/model-rule/Lorebook handling; None bypasses memory-system analysis for rows captured under it | Captured with the transcript; later changes are not retroactive |
| Archive this chat | None | Pauses/resumes review eligibility, without overriding the captured participation ceiling |
| Analyze Conversations / create package | None | Claims only material already eligible under both Archive and participation |

Archive-off material may still be captured so the review bookmark and later
state remain truthful, but it carries an archive-pause reason. Turning Archive
on clears only that reason from otherwise unprocessed rows. It never clears a
None participation reason or changes Full into General. Model rules remain on
their separate model-keyed path. General-mode Lorebook analysis may propose
general trigger material, but must reject companion-relationship content
rather than rewrite or silently reclassify it.

No new participation/recovery surface is added: no Include Earlier Messages
dialog, withheld-items screen/list, Pending category, badge, warning, or
second review destination. Existing Pending remains the sole draft-review
surface. Diagnostics count the reasons truthfully; they do not describe
ordinary exclusions as failures or recovery.

Package eligibility therefore depends on the per-capture participation value,
Archive state transitions, and durable review state—not on whether Saved
memories or Lore books happen to be injected into the chat today.
...
#### D. Archive and companion participation use existing controls

This feature adds no dialog or secondary review surface. The existing
**Archive this chat** switch pauses/resumes the archive reason on unprocessed
material. The existing companion **Memory Participation** control supplies the
Full / General / None ceiling captured with each transcript row. Changing
either setting never rewrites the other reason or retroactively changes an
older row's participation.

Do not add an Include Earlier Messages prompt, withheld-items screen/list,
Pending category, badge, warning, or recovery notice. Memory Debug/analysis
details may show reason-specific counts when useful, using ordinary factual
language rather than calling excluded material recovered, failed, or lost.

Re-enabling **Use saved memories in this chat** or **Use lore books in this
chat** also needs no dialog and does not change archived/reviewable history;
those controls change prompt injection only.
...
2. **Archive/participation behavior — RESOLVED (owner ruling, 2026-08-03):**
   use only the existing Archive switch and companion Memory Participation
   control. Participation is captured with transcript material; later changes
   are not retroactive. Archive clears/sets only its own pause reason and
   cannot override a captured None/General ceiling. Add no Include Earlier
   dialog or withheld-items UI. Source switches never alter this queue.
...
- captured companion participation plus reason-specific Archive/participation
  eligibility on transcript rows; a participation change starts a new capture
  row instead of rewriting the existing row;
...
- existing excluded transcript rows retain their state when the old reason
  cannot be proved; migration never guesses that they are Full, General,
  None, or merely Archive-paused;
...
1. **Phase 1 visible source-control copy:** the global Memory Engine
   dropdown wording shipped (owner ruling, 2026-07-28) — see §5.3. The
   Archive/participation interaction is also settled in §5.4 and receives no
   new dialog, withheld-items UI, or extra primary control. Only a genuinely
   changed visible label/helper in the existing Chat → Quick Settings controls
   returns for one focused copy decision; the four source combinations and
   the Full / General / None participation behavior are not architecture
   questions to reopen.
...
## 15. One-work-unit execution plan

This is the only work queue. Start one line item, finish it, verify it, update
its evidence here, and stop. A later item is not preparation work for the
current one.

**Current queue position:** Work Unit 0 is complete on the documentation
branch. Work Unit 1 is the only next implementation unit. Everything after it
is locked by dependency order; an implementation agent does not need to ask
about or prepare any later unit.

Every unit is a vertical completion slice, even when its user-visible result is
"the existing screen now behaves correctly." **Done** means all of the
following are in the same unit:

1. required migration/schema and recovery behavior;
2. store/service/business-rule wiring;
3. every existing call site that must use the new shared boundary;
4. the reachable UI only when its dependencies already work end to end;
5. truthful logs/status and focused automated tests;
6. green Android CI, the written device gate, and an evidence/status update in
   this file.

A helper, table, parser, screen, or green compile by itself is not a finished
feature. Do not land a button/row that leads to a later unfinished unit. Do not
leave a second activation, filing, mutation, journal, or import path "to wire
later." When a unit introduces a shared boundary, its call-site audit and
back-wiring are part of that unit's done gate.

Phase gates are absolute: Work Unit 13 unlocks Phase 2; Work Unit 24 unlocks
Phase 3; Work Unit 30 unlocks Phase 4. A failure inserts one bounded repair
unit immediately before the gate. It never authorizes parallel work in the next
phase and never creates another plan.

Owner input is also one unit at a time. If a visible conflict remains after the
invisible wiring is complete, ask the single decision named in that unit and
stop there. Do not collect a backlog of UI/UX questions or reopen choices this
document records as settled.

### Work Unit 0 — Restore one source of truth

**Status:** Complete on `codex/memory-plan-truth-repair` at `5f0a47a`; Revision
11 only finishes the content of that single plan.

**Feature:** documentation authority only; no app behavior.

- Keep this file at `Memory System/external_memory_analysis_counterplan.md`.
- Delete the competing `project-plan.md`, `plan_one_page.md`, the root June 1
  memory build plan, and the obsolete memory roadmaps/work orders moved to
  `legacy/`.
- Update `CLAUDE.md` and `Memory System/README.md` to point here and to stop
  forbidding audits of built memory features.
- Keep narrow current technical references in `Memory System/`; they are not
  delivery plans and cannot carry completion status.

**Done gate:** `rg` finds no reference that calls another memory document the
roadmap, execution plan, or source of truth. The diff contains documentation
only.

### Work Unit 1 — Step 1.1: finish Archive and companion participation semantics

**Feature:** every captured row keeps the Full / General / None ceiling that
was active when it was created, while **Archive this chat** can pause/resume
review without changing that ceiling.

- Reuse the existing companion Memory Participation control and Archive switch;
  add no new primary control, dialog, withheld-items screen/list, Pending
  category, badge, warning, or recovery surface.
- Capture participation with transcript material. A setting change starts a
  new capture row/boundary; it never changes older rows retroactively.
- Full permits normal analysis outputs. General permits general, model-rule,
  and eligible Lorebook outputs but filters companion-relationship material.
  None remains captured for truthful state but never enters Archivist,
  Associative, or Lorebook analysis.
- Archive-off sets only an archive-pause reason. Re-enabling Archive clears
  only that reason from otherwise unprocessed rows; it cannot clear None or
  turn General into Full.
- Saved-memory and Lorebook injection switches remain independent from capture
  and review. Model rules remain on their separate model-keyed path.
- The optional embedding consistency guard is backend-only: it may block/flag
  a mismatch and log the reason, but never rewrite or reclassify content; the
  explicit rules remain authoritative if the model is unavailable.
- Preserve processed/claimed rows, rename behavior, source settings, pending
  rows, and rejection keys across every toggle/setting transition.

**Focused tests:** Full / General / None at capture; setting changes before
analysis; Archive off/on for each participation value; source-switch changes;
pending/claimed/processed rows; chat rename; General companion-relationship
rejection; model-rule and Lorebook handling; optional-guard unavailable.

**Device gate:** create rows under all three participation values, toggle
Archive and the source switches, run each analysis type, and verify Pending
results plus reason-specific logs. No old None/General row gains permission.

### Work Unit 2 — Step 1.2: verify retrieval correctness as a complete feature

**Feature:** every eligible memory remains findable under a complete, partial,
missing, stale, or failed vector index.

- Keep the existing `memory-doc-v2`, relevance floor, post-filter backfill,
  policy clamps, target aliases, and updated freshness behavior.
- Add only missing regression cases; do not redesign ranking.

**Focused tests:** complete semantic index; one missing vector; no model; model
load failure; query embed failure; budget/cooldown/lore suppression backfill;
same text in different fictional scopes; title/tag/target-name lexical hits.

**Device gate:** retrieve a known memory semantically, remove one vector, prove
lexical complete-set fallback still finds it, then prove repair restores
semantic mode.

### Work Unit 3 — Step 1.3: verify the complete analysis-run lifecycle

**Feature:** one durable API analysis run survives screen loss and recovers
honestly after process death without losing or double-processing turns.

- Keep the foreground service, sealed claims, durable run row, typed scene
  context, and rename-safe rejection behavior.
- Add a test seam around claim/run transitions rather than duplicating the
  service.

**Focused tests:** append during a run; exactly one live run; Activity recreate;
service start refusal; success; partial failure; cancellation; process death;
startup reconcile; rerun; truthful found/failure counts for both analysis types.

**Device gate:** start analysis, leave the screen, turn the screen off, return,
then repeat with a forced process restart. The history and Pending counts must
match committed work exactly.

### Work Unit 4 — Step 1.4: finish index repair and diagnostics

**Feature:** a partial index repairs itself once, fails visibly, and never hides
eligible memories while repairing.

- Keep scoped vector loads and the existing caches.
- Make repair state/failure inspectable after the turn that discovered it;
  ordinary logs may carry detail, but a transient callback alone is not the
  durable diagnostic required here.
- Ensure one repair worker, retry after failure, model/document-version
  invalidation, and no stale cached vector after edit/model change.

**Focused tests:** single-flight scheduling, per-row failure and later retry,
model switch, document-version switch, memory edit, lore edit, comparison-cache
invalidation, and lexical retrieval throughout a partial repair.

**Device gate:** corrupt one vector, trigger retrieval, inspect the diagnostic,
and confirm repair clears the condition.

### Work Unit 5 — Step 1.5A: create the shared filing and evidence boundary

**Feature:** every API-created draft has trustworthy source evidence and passes
one filing gate before it reaches Pending.

- Add one shared filing service used by the API path first and later reused by
  computer import. Do not merge an abandoned branch wholesale.
- Validate normalized identity, scope, stable targets, current statuses,
  rejected-draft identity, participation ceiling, and provenance in one
  transaction.
- Store source chat UID/ID/name at filing time, source transcript row IDs,
  source timestamp, bounded evidence excerpt, run ID, captured participation,
  and transport.
- Send stable target IDs in the API prompt. Duplicate target names are an
  unresolved placement state, never `.take(1)`.
- Make **Entered by hand**, **Imported**, and **Learned from chat** depend on
  origin, while evidence quality stays separate.

**Focused tests:** valid filing; Full/General/None filing; exact duplicate;
different kind; archived or superseded match; duplicate target names; missing
target; rejected draft; rename-safe evidence; partial save failure; provenance
display.

**Device gate:** analyze one Full and one General conversation, open Pending →
Information, and verify the source, participation, and excerpt against the chat.

### Work Unit 6 — Step 1.5B: use one shared acceptance gate everywhere

**Feature:** quick Save, editor approval, and future comparison/import
resolutions cannot activate a stale, misplaced, or duplicate draft.

- Replace direct `setMemoryStatus(..., "active")` acceptance with one service
  that rechecks the draft, participation ceiling, placement, stable targets,
  exact identity, and current library inside the transaction.
- Route quick Save, editor approval, Add to Card where applicable, and the
  acceptance primitive used by later Possible Match/import work through that
  boundary.
- If a match appeared after the card rendered, keep the draft Pending and send
  it to Review; never activate it and explain later.
- Complete the back-wiring in this unit: no existing activation call site may
  bypass the service before the unit is marked done.

**Focused tests:** race between render and tap; edit-before-save; target deleted;
target renamed; duplicate created in another path; repeated tap; app restart;
General/None ceiling violation; audit that all activation call sites use the
shared service.

**Device gate:** hold a Pending card open, create a conflicting memory through
another path, then tap Save and confirm the draft remains Pending for Review.

### Work Unit 7 — Step 1.5C: make Possible Match actions safe and escapable

**Depends on:** Work Units 5–6. This unit consumes the shared filing and
acceptance services; it must not build a separate comparison transaction path.

**Feature:** the user can resolve a flagged suggestion without losing an old
memory or becoming trapped in Review.

- Replace the permanent-delete behavior behind **Save & Replace** with retained
  replacement/supersession history. No comparison action hard-deletes an old
  memory.
- Provide **Keep Existing** and **Delete Suggestion** outcomes. Preserve Keep
  Both behavior; editing an old memory is optional, not the price of keeping
  both records.
- Re-read the proposal and every selected match inside the shared transaction.
  If content, placement, kind, target, status, evidence, participation, or
  identity changed, apply nothing and leave a specific Pending/conflict state.
- Preserve multiple-match selection, source/evidence controls, and the normal
  full-page comparison layout.

**Focused tests:** every action with one/multiple matches; stale proposal;
stale selected record; archived/superseded match; repeated tap; interruption at
the transaction boundary; assertion that comparison never calls hard delete or
bypasses the shared acceptance gate.

**Device gate:** exercise each resolution, then browse active, archived, and
superseded history to prove no unintended record vanished.

### Work Unit 8 — Step 1.5D: complete Possible Match source coverage

**Feature:** a proposed associative memory can be compared with relevant active,
draft, archived, and superseded memories plus read-only Lorebook entries and
roleplay-card content.

- Keep the fiction wall and placement-aware eligibility.
- Add typed read-only comparison candidates for Lorebooks and cards. They may
  be kept as authored content or used to reject/keep-separate the proposal;
  analysis never edits them.
- Do not show a similarity percentage or call similarity a contradiction.
- Calibrate candidate retrieval with fixtures before changing the current
  threshold.

**Focused tests:** exact/semantic active memory; draft; archived; superseded;
Lorebook overlap; card overlap; disjoint worlds; negation; multiple sources;
no embedding model.

**Device gate:** one Pending proposal that produces each candidate type, with
source/evidence visible and no mutation until a chosen resolution.

### Work Unit 9 — Step 1.6: verify Lorebook runtime retrieval and logs

**Feature:** selected Lorebooks inject deterministically, deduplicate exactly,
respect budget, and say what matched, entered, or was cut.

- Keep current trigger semantics (latest user message only), core-first order,
  and first-occurrence-wins exact content dedup.
- Ensure source failure, duplicate, entry limit, and character-budget cuts each
  carry entry ID, book ID, and a truthful reason.

**Focused tests:** batched query ordering, duplicate content across books,
multiple triggers, disabled entry, core priority, every cut reason, source
failure, and debug-log rendering.

**Device gate:** two active books with one duplicate and enough matches to force
a cut; compare the actual prompt with the debug log.

### Work Unit 10 — Step 1.7A: verify Lorebook Memories analysis filing

**Feature:** a Lorebook Memories run reliably produces only trigger-bearing
Lorebook Suggestions and never associative drafts.

- Keep the current two analysis types and their separate editable prompt slots.
- Bind selection, service input, durable run row, rerun, cancellation, history,
  parser, found count, and Pending destination to the run's own stored type.
- A malformed entry, missing content, or empty triggers is rejected and counted
  truthfully.

**Focused tests:** prompt contract; wrapped JSON; mixed valid/invalid entries;
all-invalid result; associative/lore type isolation; rerun after changing the
picker; partial failure; cancellation; process recovery.

**Device gate:** run each analysis type over the same small conversation and
prove its suggestions appear only in the correct Pending area.

### Work Unit 11 — Step 1.7B: make Lorebook Pending approval crash-safe

**Feature:** assigning, editing, approving, or deleting a Lorebook Suggestion is
reachable, idempotent, and cannot duplicate an entry after interruption.

- Preserve the recorded Lorebooks/Pending split, suggestion content + triggers,
  no separate title, existing/new destination, full-page new-book flow, and
  Edit/Approve/Delete controls.
- Use the app's actual canonical dropdown behavior for destination selection or
  document why the existing list chooser is intentionally equivalent.
- Add a stable approval/commit ledger spanning the Lorebook store and pending
  suggestion store. Repeated approval and restart after either write are no-ops,
  never duplicate entries.
- Revalidate the destination and edited content/triggers at commit time.

**Focused tests:** split hidden/visible; direct View navigation; assign existing;
create new; cancel creation; edit; approve; delete/rejection suppression;
destination deleted; failure before entry write; failure after entry write;
repeated approval; restart recovery.

**Device gate:** complete every control from the Memory Assistant result link,
including a forced interruption during approval.

### Work Unit 12 — Step 1.7C: settle the missing-model notice once

**Feature:** one accurate, non-contradictory explanation appears in Memory
Browser only when Associative Memories exist but cannot enter chats.

- Present the two conflicting recorded behaviors together: Revision 7's
  persistent inline banner and the later per-visit dismissible reminder.
- Ask one binary behavior question at the start of this unit. Do not reopen the
  Lorebook picker, Pending layout, or analysis wording.
- Implement the chosen behavior, link to Advanced Memory Settings, hide it when
  no saved memories exist, and remove it automatically once a usable model is
  installed.

**Focused tests:** no memories; saved memories/no model; draft only; model
installed; model removed; return from settings; process restart.

**Device gate:** exercise each state and confirm the notice never lies about
analysis or Lorebook Memories.

### Work Unit 13 — Phase 1 completion gate

Run the §5.9 matrix plus every focused test and device gate above. Re-audit the
actual entry points, not only helper classes. Mark Steps 1.1–1.7 Complete only
where the strict status definition at the top of this file is satisfied. Any
failure creates one new bounded work unit; it does not reopen the whole plan.

**Hard gate:** Phase 2 may begin only after Work Unit 13 confirms **every part
of Steps 1.1–1.7 Complete**—code present, focused automation, device-verified,
and with no known gap against this plan. "Release-blocking parts complete" is
not enough. Lorebook approval, comparison coverage, diagnostics, and the
missing-model behavior cannot hang unfinished behind package work.

### Phase 2 — Portable Memory Package

No Phase 2 unit starts until Work Unit 13 passes. Phase 2 builds the package
from the inside out; the Memory Assistant entry row appears only after the
package can be created, verified, written, and tracked end to end.

### Work Unit 14 — Freeze the `.sgmemory` contract and golden fixtures

**Feature:** one versioned package contract can be implemented and validated
without opening another planning document.

- Materialize the §9.4 manifest, record envelopes, normalization versions,
  semantic hashes, instructions, schemas, and result template as repository
  assets/pure models.
- Freeze required versus optional capabilities, package purposes
  (`archive_review`, `library_maintenance`), limits, stable-ID rules, and the
  no-database/no-key/no-vector boundary.
- Keep backup, recovery, chat export, and exchange formats separate.

**Focused tests:** schema self-validation; canonical fixture hashes across
Unicode/field/target order; supported/unsupported capability fixtures; every
manifest reference resolves; backup codecs remain behavior-compatible.

**Done gate:** a fixture package and fixture result validate without Android UI
or a repository-aware agent. No export entry point exists yet.

### Work Unit 15 — Add immutable conversation identity and durable evidence

**Depends on:** Work Unit 14's ID/evidence envelope.

**Feature:** a chat rename can never break exported evidence or returned
suggestions.

- Add immutable `conversation_uid` with safe legacy aliases/backfill.
- Add/finish durable `memory_evidence` using transcript row IDs, timestamps,
  quote hashes, bounded excerpts, captured participation, and source transport.
- Route capture, rename, API filing, editor display, backup/restore, and
  rejection identity through the stable UID without fabricating missing
  historical evidence.

**Focused tests:** legacy migration; repeated rename; duplicate display names;
deleted/missing chat; unambiguous and ambiguous provenance backfill; seed and
backup compatibility; evidence survives process restart.

**Device gate:** create, analyze, rename, and reopen a chat; Pending evidence
still opens the correct source and quote.

### Work Unit 16 — Add dormant exchange tracking and first-workspace baseline

**Feature:** change journals and tombstones exist only when a computer workspace
needs them, and every relevant mutation path feeds them exactly once.

- Add `exchange_workspaces`, memory/lore/target change feeds, tombstones, and
  tracking-enabled state defined in §10.2c.
- Keep hooks dormant before the first successful workspace; a failed first
  creation removes exchange-only state and disables tracking.
- Back-wire the Phase 1 semantic mutation/invalidation services, editor,
  approval, import/restore, archive/supersede, and Lorebook/target mutations.
  No relevant mutation call site may bypass journal/tombstone recording while
  tracking is active.
- The full snapshot is the baseline; do not synthesize one journal event per
  existing record.

**Focused tests:** no-workspace dormancy; successful baseline; failed baseline
rollback; one event per mutation; rename/archive/delete/restore; store failure;
multiple rapid edits; journal pruning preconditions.

**Device gate:** prove ordinary phone use creates no exchange history before a
workspace, then create/remove a test workspace and inspect tracking transitions.

### Work Unit 17 — Freeze export snapshots and arbitrate transcript claims

**Depends on:** Work Units 3, 15, and 16.

**Feature:** API analysis and one computer review may coexist without ever
reviewing the same transcript row.

- Add the package/item ledger and one atomic claim operation shared with the
  API run path.
- Permit at most one active API run and one outstanding computer archive-review
  package; each claims only unclaimed eligible rows.
- Seal claimed rows; turns created during a run/package start new unclaimed
  rows. Library-maintenance packages claim no transcripts.
- Freeze a source snapshot and initial cursor vector so a racing mutation is in
  either that verified snapshot or the next journal delta, never neither.

**Focused tests:** API-first/package-first races; simultaneous claim attempt;
mid-run append; cancel; process death; library-only package; Archive and
participation eligibility; first-export mutation race.

**Device gate:** leave one API run and one package outstanding over disjoint
chats, add new turns, and verify the ledgers and visible counts exactly.

### Work Unit 18 — Build the archive-review source dataset

**Feature:** selected eligible chats compile into a complete, evidence-linked,
read-only review dataset before any file is written.

- Resolve selected/new chats through immutable UIDs, captured participation,
  Archive eligibility, and the frozen claims from Work Unit 17.
- Emit normalized turns, scene context, target catalog, relevant existing
  memories, and read-only Lorebook/card references needed to avoid duplicates.
- Every exported quote/evidence reference resolves and verifies; unavailable
  chats block the affected item instead of becoming an empty conversation.
- Preserve user-only/incomplete-assistant capture states truthfully.

**Focused tests:** Full/General/None; Archive pause; selected/new chat scopes;
rename; unavailable/corrupt chat; cross-world isolation; evidence/hash mismatch;
new turns outside snapshot; no unrelated chat leakage.

**Done gate:** a pure archive-review dataset passes exact count, scope, target,
and evidence assertions without a ZIP writer or UI.

### Work Unit 19 — Build the library-maintenance source dataset

**Feature:** all or selected library scopes compile into a searchable snapshot
that cannot grant external write authority.

- Emit active/draft/archived/superseded memory envelopes, stable target catalog,
  revision events, tombstones, and read-only Lorebook/card references defined
  by the selected scope.
- Keep user personas/companion cards as context, not writable targets.
- Preserve origin/provenance and semantic hashes; omit device-local prior-state
  bodies, vectors, databases, keys, and unrelated settings.
- If a required store is unavailable, block completeness rather than masking it
  as an empty library.

**Focused tests:** all/selected scopes; duplicate names/stable IDs; every memory
status; tombstones; unavailable memory/lore store; no-chat library package;
privacy inventory; exact searchable JSONL fixtures.

**Done gate:** a pure library-maintenance dataset validates independently of
the archive-review compiler.

### Work Unit 20 — Assemble and adversarially validate the full package

**Depends on:** Work Units 18–19.

**Feature:** either source dataset becomes one deterministic, bounded,
self-describing `.sgmemory` package.

- Assemble the exact §9.4 tree with manifest, schemas, instructions, catalog,
  evidence, checksums, limits, and blank result template.
- Validate the assembled package with the app's compiled schemas and recomputed
  checksums before it can be written or recorded as outstanding.
- Treat all chat/memory text as untrusted data, never instructions.

**Focused tests:** exact record counts; deterministic fixture output; missing/
duplicate path; ZIP traversal, symlink, compression bomb, excessive count/
depth/string/file; checksum mismatch; unknown required capability; privacy
inventory proving no keys/databases/vectors.

**Done gate:** both package purposes survive the adversarial suite and can be
opened using only their bundled manifest/instructions.

### Work Unit 21 — Write atomically and maintain the outstanding-package ledger

**Feature:** a verified package reaches a user-selected SAF destination once,
and cancellation/replacement never lies about claims or cursors.

- Write to a temporary destination, verify the final bytes, then publish and
  mark outstanding in the same recoverable workflow.
- Do not advance baselines/cursors or leave claims/outstanding state after a
  failed or abandoned write. Startup reconciles temp files and ledger states.
- Implement share/reopen, cancel, and replace primitives. Cancel releases only
  that package's claims and never deletes new transcript rows; replace creates
  and verifies the successor before retiring the old package.
- Enforce the package cap by requiring narrower selection, never silent
  omission.

**Focused tests:** SAF cancellation/denial; short write; byte verification
failure; app death at each state; successful reopen/share; cancel; replace;
failed first workspace cleanup; cursor/claim assertions.

**Device gate:** create, share, cancel, and replace a package through the Android
picker, including one interrupted write; ledger and chat eligibility remain
truthful after restart.

### Work Unit 22 — Add task and source selection to Memory Assistant

**Depends on:** Work Units 14–21; no placeholder row may land earlier.

**Feature:** **Review on a Computer** opens a complete choice screen for Review
chats or Check memory library and shows exactly what will be included.

- Reuse the §9.8 entry point and existing app styles/theming.
- Chat review selects only eligible chats/new material. Library maintenance
  selects all or explicit scopes/targets. Show read-only Lorebook/card coverage
  and the plaintext state.
- At this unit's start, ask at most one owner question: which of the two existing
  task choices is the initial default. Do not reopen package architecture,
  source controls, or unrelated wording.
- A user cannot proceed until the source compiler says the chosen task is
  complete and valid.

**Focused tests:** both tasks; empty/partial selection; unavailable source;
Archive/participation filtering; rotation/restart; theme variants; accessibility.

**Device gate:** enter from Memory Assistant, create each selection, back out,
return, and verify the content summary against the dataset counts.

### Work Unit 23 — Add privacy confirmation, destination, and outstanding state

**Feature:** the user reviews plaintext exposure, chooses a destination, receives
persistent verified success, and can manage the one outstanding package.

- At this unit's start, ask at most one owner question: approve/correct the
  exact plaintext disclosure. Do not combine it with Phase 4 copy decisions.
- Show **What this file contains**, then the disclosure, then SAF destination.
- On success show filename/date/task/item counts and next action persistently;
  never rely on a toast.
- Show created time, frozen items, new turns waiting, Share, future Import
  entry, Cancel, and Replace for the outstanding package. Import remains
  disabled/unavailable until Phase 3 produces a valid result checker; do not
  lead to a dead screen.

**Focused tests:** disclosure required; cancel at each step; successful state;
outstanding/restart; API run coexistence; zero unclaimed chats; share/cancel/
replace; no inaccessible future navigation.

**Device gate:** complete the full selection → disclosure → SAF → success flow
and every outstanding-package action on the shipping theme.

### Work Unit 24 — Phase 2 completion gate

Run §10.2e plus Work Units 14–23. Verify both package purposes, every migration,
every mutation hook, atomic file recovery, consent/participation boundaries,
privacy inventory, and old backup compatibility. Two independent file-capable
agents must open the same fixture package and produce schema-valid sample result
files without repository knowledge; this is package portability proof, not yet
the full workflow trial.

**Hard gate:** Phase 3 does not begin until the entire full-package creation and
outstanding-state feature is Complete. Any failure becomes one bounded repair
unit inserted before Work Unit 25.

### Phase 3 — Computer Workflow Proof

No Phase 3 unit changes memories. It proves that ordinary file-capable agents
can use the package and that the phone can recognize trustworthy results before
Phase 4 adds import authority.

### Work Unit 25 — Materialize the desktop workspace and instructions

**Feature:** a user can place/extract a full package in a private folder and an
agent can validate/materialize a disposable workspace using only bundled files.

- Finalize provider-neutral `agent_workflow.md`, safety/scope instructions,
  shared retrieval/normalization specs, and golden examples.
- Define deterministic full-workspace materialization and rebuildable local
  index behavior. The Android phone remains the only authority.
- Ship a repository validator/linter for package/result fixtures; do not build a
  desktop executable unless Work Unit 29 proves ordinary agents need one.

**Focused tests:** clean/corrupt workspace; repeated materialization; deleted
index rebuild; missing file; stale snapshot; lexical/no-model path; untrusted
text cannot override instructions.

**Done gate:** delete the workspace and recreate it from the same package with
identical authoritative record hashes.

### Work Unit 26 — Prove the archive-review playbook

**Feature:** a file-capable agent can find relevant memory/lore/evidence for
each frozen chat and return terminal item status plus cited new-memory
suggestions.

- Implement/test the search order and scope wall in §9.5.3.
- Require evidence refs, target IDs, kinds, item/input hashes, and explicit
  complete/no-change/failed states.
- General rows may not produce companion-relationship suggestions; None rows
  cannot appear. Lorebooks/cards remain read-only comparison context.

**Focused tests:** exact/near match; evidence neighborhood; disjoint worlds;
General filtering; valid add; no-change; failed/skipped; fake target/quote/ID;
lexical-only search.

**Done gate:** golden review questions return all must-include and zero forbidden
IDs, and every frozen item receives a truthful terminal state.

### Work Unit 27 — Prove the library-maintenance playbook

**Feature:** a file-capable agent can inspect a library and return strict
add/revise/merge/retire proposals without mutating authored content.

- Require exact base revisions/hashes, proposed final record, evidence/related
  refs, target IDs, and concise rationale for every operation.
- Lorebooks, cards, personas, and model rules are comparison context only.
- Similarity suggests a Possible Match; it is never a percentage, automatic
  contradiction, merge, expiry, status change, or target change.

**Focused tests:** valid example per operation; exact/near/lore/card overlap;
archived/superseded state; bad base/hash/target/evidence; unknown operation;
duplicate proposal ID; no-change distinct from failure.

**Done gate:** golden maintenance fixtures validate with no direct-edit or
unsupported-operation escape hatch.

### Work Unit 28 — Validate and preview result files on the phone

**Feature:** **Check result file** says what a returned file contains and why
invalid items failed, while making zero MemoryStore/LoreBookStore mutations.

- Copy the chosen result to bounded private staging, strict-parse it, validate
  schema/capabilities/package/workspace/snapshot/item/input/target/evidence
  references, and recompute payload hashes.
- Present counts for new/edit/merge/archive/no-change/conflict/failed/skipped
  items plus bounded actionable errors. This is a preview only.
- Store only validation metadata/hashes if history is needed; discard staging
  according to the retention rule.
- Wire the outstanding package's Import/Check action to this complete preview.

**Focused tests:** every valid operation; prose-wrapped JSON; unknown fields/
capability; stale/wrong package; bad hashes/evidence/targets; duplicates; size/
depth limits; repeated preview; assertion of zero authoritative-store writes.

**Device gate:** choose valid, partial, and malicious result fixtures; compare
the preview counts/errors with the files and prove the memory library is byte/
record unchanged.

### Work Unit 29 — Run real cross-agent workflow trials

**Feature:** at least two independent file-capable agents can complete both
package purposes without a custom API key and return results the phone previews
correctly.

- Trial two agents with a clean full package, lexical/no-model instructions,
  stale/missing workspace, and adversarial scope/evidence questions.
- Record search recall, forbidden leakage, invalid-output causes, and manual
  correction steps in this file.
- If ordinary file-agent operation fails because validation/materialization/
  search is too fragile, insert one bounded **Work Unit 29R** for the smallest
  cross-platform `sg-memory` helper needed. Do not build a helper preemptively
  and do not hide a failed trial by relaxing the result contract.

**Done gate:** both agents produce schema-valid results; no-change, skipped, and
failed remain distinguishable; zero forbidden-scope items are proposed.

### Work Unit 30 — Phase 3 completion gate

Run §10.3f plus Work Units 25–29. A user must be able to export, transfer,
materialize, review, return one result file, and see a trustworthy phone preview
without changing any memory. Archive-review and library-maintenance results,
lexical fallback, evidence, scope, stale package, and malformed output must all
pass.

**Hard gate:** Phase 4 import code does not begin until this entire no-mutation
round trip is Complete.

### Phase 4 — Import and Reconciliation

Phase 4 turns the already-proven result contract into Pending suggestions. It
builds one operation at a time on one shared classification/mutation boundary;
the computer never receives direct write authority.

### Work Unit 31 — Stage imports with a replay-safe session/item ledger

**Feature:** selecting a result creates one durable, resumable import session
without touching memories or Pending.

- Reuse Work Unit 28 validation, then persist package/result/session/item IDs,
  hashes, staged payload refs, and state transitions.
- One idempotent transaction per item or atomic merge group; a repeated file or
  process restart resumes/no-ops rather than duplicating work.
- Unverified staging is discarded; validated/uncommitted items remain resumable.

**Focused tests:** first import/replay; duplicate proposal IDs; partial ledger
write; process death at every state; staging cleanup/expiry; wrong package;
multiple picker attempts; zero MemoryStore mutation.

**Device gate:** interrupt import before/after validation, restart, and verify the
same session resumes with unchanged memory/Pending counts.

### Work Unit 32 — Classify every item against current phone state

**Depends on:** Work Unit 31.

**Feature:** every returned suggestion receives one truthful current-state
classification before a draft/proposal can be created.

- Implement every §9.6.2 row: replay, already saved/pending, Possible Match,
  clean, unchanged, stale conflict, deleted conflict, target conflict, invalid
  evidence, and incompatible.
- Re-read current revisions/status/targets/identity/evidence per item; unrelated
  phone edits do not invalidate clean items.
- Persist classification and bounded details with the item ledger; do not
  coerce an invalid item into a weaker suggestion.

**Focused tests:** every classification row; phone edit/rename/archive/delete;
duplicate created after export; missing target; stale package with unrelated
clean add; repeated classification; process death.

**Done gate:** classification fixtures match the §9.6.2 matrix exactly and
still create no Pending or active records.

### Work Unit 33 — Extend the shared semantic mutation and apply-batch boundary

**Feature:** every future approved computer operation uses one atomic service
that preserves evidence/history, invalidates retrieval state, journals the
change, and can support conditional Undo.

- Extend the Phase 1 acceptance service for add/revise/merge/retire without
  creating transport-specific mutation paths.
- Revalidate targets/scope/kind/status/revisions/hashes/evidence at commit;
  write complete joins, `updated_at`, exchange event, vector/cooldown
  invalidation, evidence links, and before/after apply-batch data atomically.
- Prefer archive/supersede to delete. A merge is one transaction across every
  source and survivor.
- Back-wire all existing semantic mutation paths that must share these
  invariants; audit call sites before marking done.

**Focused tests:** each operation primitive; stale tap; repeated apply; partial
failure rollback; merge atomicity; journal/invalidation/evidence; before-image
and after hash; bypass audit.

**Done gate:** pure service tests prove no operation can partially commit or
activate without the shared gate. No computer proposal UI is added yet.

### Work Unit 34 — File new-memory results into existing Pending

**Feature:** clean external add suggestions become ordinary Pending memory
drafts with computer origin and verified evidence—never active memories and
never a second queue.

- Route only clean add items through Work Unit 5's filing boundary and Work
  Unit 6's acceptance gate.
- Already-saved/pending items remain no-ops; archived/superseded/different-kind
  matches go to the existing Possible Match path.
- Reuse existing Pending Edit/Accept/Delete/Add to Card behavior and origin/
  evidence display. Ask one focused wording question only if the existing
  origin vocabulary cannot represent computer review accurately.
- Commit the item ledger transition in the same transaction as draft creation.

**Focused tests:** clean add; duplicate; Possible Match; bad target/evidence;
General ceiling; replay; repeated tap; process death; assertion no direct active
insert and no second Pending store.

**Device gate:** import add/no-change/match fixtures, review each in existing
Pending, and verify the source/evidence and ledger.

### Work Unit 35 — Add Suggested Edit review and approval

**Feature:** a clean revise proposal shows current versus suggested fields and
can be kept, edited, or applied through the shared mutation service.

- Add one operation-specific Pending card/detail using the existing queue.
- Show current/suggested placement, evidence, rationale, and changed fields;
  never treat unchanged text as an edit.
- Keep current rejects/withdraws the proposal. Edit suggestion creates a new
  current-base payload. Apply revalidates and commits atomically.

**Focused tests:** every field/target/status change; unchanged; stale tap;
edited proposal; keep current; replay; restart; vector/journal/evidence effects.

**Device gate:** exercise Keep current, Edit suggestion, and Apply, then inspect
history, retrieval, evidence, and the import ledger.

### Work Unit 36 — Add Suggested Merge review and approval

**Feature:** one merge proposal shows every source and survivor and either keeps
them separate or performs one recoverable atomic merge.

- Show all source memories, proposed survivor/final body, placement, evidence,
  and archived/superseded lineage.
- Keep separate rejects without mutation. Edit merge rebuilds the proposal on
  the current base. Merge revalidates every source and uses Work Unit 33.
- Never hard-delete sources or perform an automatic semantic merge.

**Focused tests:** two/many sources; cross-scope rejection; stale/deleted source;
edit; keep separate; partial failure; repeated tap; superseded retrieval
exclusion; full lineage and rollback data.

**Device gate:** complete all actions and prove either all merge effects commit
or none do; every old record remains reviewable in history.

### Work Unit 37 — Add Archive Suggestion review and approval

**Feature:** a retire proposal can be kept or archived without creating a new
delete shortcut.

- Show the current memory, evidence, later/replacement context, and reason.
- Keep rejects with no mutation. Archive revalidates then uses the shared
  archive/supersede path; the existing explicit Delete flow remains separate.
- Do not expose internal `retire` wording or imply expiry/automatic cleanup.

**Focused tests:** active/already archived/superseded/deleted; stale proposal;
keep; archive; repeated tap; journal/tombstone/invalidation/history.

**Device gate:** keep one suggestion and archive another; confirm retrieval and
history behave correctly and no hard delete occurred.

### Work Unit 38 — Add stale/deleted/target conflict review

**Feature:** a conflict shows Base at export, Phone now, and Suggested without
overwriting newer phone work.

- Add the operation-specific conflict card in existing Pending.
- Offer Keep phone/dismiss and Refresh computer package. A manual new proposal
  may be created only from the current base; do not auto-merge text.
- Missing/deleted targets remain conflicts and never recreate records
  implicitly. Clean unrelated items continue.
- At this unit's start, ask at most one owner question for the final action
  labels; do not reopen edit/merge/archive architecture.

**Focused tests:** stale revise/merge/retire; deleted base; invalid target;
unrelated clean add; keep phone; refresh; manual current-base proposal; restart.

**Device gate:** edit a phone memory after export, import the old suggestion,
and prove no action can overwrite the newer phone value silently.

### Work Unit 39 — Finish partial-import status and transcript claim release

**Feature:** the outstanding-package screen tells exactly what committed,
changed nothing, conflicted, failed, or remains, and releases only completed
work.

- Drive package/item/session states from the same transactions as draft/
  proposal effects.
- Mark a frozen transcript processed only for a terminal complete/no-change
  item whose filing/proposal commit succeeded. Failed/conflict items stay
  retryable; new turns remain unclaimed.
- Persist the import summary and route each count to existing Pending/conflict
  details. Never call partial success complete.
- Cancellation releases claims but does not delete accumulated new rows or
  already approved changes.

**Focused tests:** all terminal/mixed states; death after each item; retry failed;
conflict resolution; cancel partial session; repeated result; claim/bookmark
counts; API run coexistence.

**Device gate:** import a mixed-result fixture, restart mid-run, finish/cancel,
and compare summary, Pending, transcripts, and claims item by item.

### Work Unit 40 — Add conditional Undo and pre-approval import cancel

**Feature:** the user can remove unapproved imported suggestions or undo an
approved apply batch only while doing so cannot overwrite newer work.

- Before approval, cancel removes/rejects only that session's staged rows and
  preserves independent Pending work.
- After approval, Undo restores the complete before state only when every
  affected record still matches the expected after hash; otherwise show a
  conflict and apply nothing.
- A merge Undo restores every affected record atomically. Do not promise
  hard-delete rollback.

**Focused tests:** cancel before approval; add/revise/merge/retire Undo; later
phone edit conflict; repeated Undo; partial failure; app death; exchange journal,
vectors, cooldowns, evidence, and history after restore.

**Device gate:** undo each operation, then alter one approved record and prove
Undo refuses to overwrite it.

### Work Unit 41 — Add full-to-delta computer workspace updates

**Feature:** **Update computer workspace** emits and applies ordered changes and
tombstones, while a broken chain safely falls back to a new full package.

- Read each source after its acknowledged cursor; emit current bodies for
  upserts, tombstones for deletes, changed evidence/chat fingerprints, required
  base snapshot, and new cursor vector.
- Advance phone cursors only after a verified atomic write. The desktop
  materializer applies deltas in order and returns the analyzed snapshot ID;
  file creation alone is not acknowledgment.
- Validate duplicate/missing/out-of-order deltas and pruned journals. Recovery
  is **Create new full package**, never an inferred merge from an unknown mirror.
- Keep one workspace initially; multiple paired desktops remain out of scope.

**Focused tests:** full + ordered deltas; mutation during delta; duplicate/
missing/out-of-order delta; tombstone; lost/deleted workspace; pruned journal;
stale result from old snapshot; atomic cursor advancement.

**Device gate:** create a full workspace, edit/add/archive/delete phone records,
apply two deltas, then delete one delta and prove the app requires a fresh full
package without harming phone data.

### Work Unit 42 — Enforce exchange version compatibility and staged migration

**Feature:** supported older/newer minor files migrate safely in private staging,
while incompatible major/capability changes are rejected before store changes.

- Implement §9.6.8 using compiled trusted schemas; package-supplied schemas are
  descriptive, not executable authority.
- Preserve stable IDs/hashes across app upgrades. Never rewrite the user's
  source file during migration.
- Retain golden packages/results for every supported version and adversarial
  future versions.

**Focused tests:** every supported minor; newer optional field; missing required
capability; old minor migration; unsupported major; unknown record/operation;
normalization/document-version change; zero mutation on reject.

**Device gate:** preview/import one supported old fixture and reject one future
incompatible fixture with an actionable persistent reason.

### Work Unit 43 — Decide and, only if required, add the encrypted wrapper

**Feature:** the release has one explicit encryption policy rather than an
unfinished half-option.

- Ask one binary owner decision at this unit: plaintext-first interoperability,
  or encryption required before broader release.
- If plaintext-first is chosen, record encryption as deferred and keep the
  approved disclosure from Work Unit 23; do not add dormant crypto UI/code.
- If encryption is required, implement only the §9.7.2 wrapper/helper, with
  authenticated encryption, key-loss behavior, bounded decryption staging, and
  no claim that content stays encrypted after a cloud agent reads it.

**Focused tests if built:** correct/wrong/lost key; tamper; replay; oversized
ciphertext; process death; plaintext and encrypted package parity; no secret in
logs/backups.

**Device gate:** either confirm the plaintext-only release state has no dead
encryption controls, or complete encrypt → desktop open → result → phone import
plus lost-key recovery.

### Work Unit 44 — Phase 4 full round-trip completion gate

Run §10.4e and every Work Unit 31–43 test/device gate. Verify full export →
desktop review → strict import → existing Pending → add/edit/merge/archive/
conflict decisions → conditional Undo → ordered delta → next result. Repeat
with replay, process death at every commit boundary, stale phone changes,
deleted targets, lost desktop cache, and the selected encryption policy.

**Hard gate:** Phase 4 is Complete only if no route can activate, merge,
archive, supersede, delete, retarget, or mark a transcript processed without the
required explicit user action and shared transaction boundary.

### Work Unit 45 — Fresh independent correctness review

Run the §8 review against production entry points, migrations, schemas,
adversarial fixtures, call-site audits, CI, and all recorded device evidence.
The reviewer checks the four source combinations, Full / General / None,
Archive precedence, fiction/scope walls, evidence lineage, idempotency,
rollback, journal/delta recovery, privacy inventory, and compatibility.

Any failure becomes one bounded repair work unit inserted immediately before
release. It does not create a second roadmap and does not reopen already-settled
UI choices.
