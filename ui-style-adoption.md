...
| Voice Settings | `activity_voice_settings.xml` | Shared | Header plus shared toggle rows (Always Speak Responses, Auto Send, Automatic Language Detection, Read Formatting Language); the four local plain-text section headers (Text-to-speech, Speech-to-text, Hands-free & voice activity, Debug logging) were removed — no shared style backed them; the Silent Mode tile and all its preference/wiring were removed entirely (owner decision); the Voice Language tile was replaced by a `Widget.App.Row.Selector` row that opens the new full-screen Select Voice Language picker | Partial | Remaining tile-grid body components (TTS engine/voice, STT engine, Hands-Free Timing, VAD Method) not audited; a few local attributes remain. |
| Select Voice Language | `fragment_select_language.xml` | Shared | Full-screen `UI.Material` window with `ActionBar`/`BackButton`/`Title`; list rows use `Widget.App.PickList.Row`/`TextAppearance.App.PickList.Unselected`/`.Selected` (`LanguageListAdapter`/`view_language_pick_item.xml`); opened from Voice & Speech's Voice Language `Widget.App.Row.Selector` row | Shared | None. Tapping a row selects it and dismisses immediately, matching Select AI Model's interaction; unlike that screen, this one has no search field or favorites split, so there is no local/unaudited piece left. |
...
| Picker sheets (Voice, Activation Prompt, System) | `fragment_select_voice.xml`, `fragment_activation_prompt.xml`, `fragment_system.xml` | N/A | Pick tiles / option lists | Unconverted | The owner-approved "checked tile" pick pattern exists for some pickers; audit each against it before converting. Select Voice Language was converted separately (see its own entry above) to the new full-screen `Widget.App.PickList.Row` family — use that as the reference when converting these. |
