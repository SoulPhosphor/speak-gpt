...
| Memory Filters Panel | `activity_memory_filter_panel.xml` | Shared | Close-panel header (`Title.LeftAligned`/`CloseButton`); every filter row (Sort, Source, Superseded Memories, and the multi-select Scope/Type/Tags) is a `Widget.App.Dropdown.Label`/`.Value` row opening an anchored `ListPopupWindow` (built in code from `view_memory_filter_dropdown.xml`); Reset Filters is `AppButton.Destructive`. Rows carry no filled background. | Partial | The selection chips below the multi-select rows are still a local Chip look (`accent_100`, 10dp corners), not a shared style — the one remaining local element. Multi-select rows grey out (italic, dimmed, disabled) options absent from the loaded set. |
