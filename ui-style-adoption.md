...
| Voice Settings | `activity_voice_settings.xml` | Shared | Header plus shared toggle rows (Always Speak Responses, Auto Send, Read Formatting Language); the four local plain-text section headers (Text-to-speech, Speech-to-text, Hands-free & voice activity, Debug logging) were removed — no shared style backed them | Partial | Remaining tile-grid body components not audited; a few local attributes remain. |
