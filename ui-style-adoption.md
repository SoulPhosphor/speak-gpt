...
| Edit Lorebook | `activity_edit_lorebook.xml` | Shared | Header fully shared (`ActionBar`/`BackButton`/`Title.NearBack`/`ChainedButton` Delete (`ic_delete` trashcan)/`SecondaryButton` Save); green save-flash + `DiscardChangesDialog` back-out; delete confirm uses the shared two-button dialog (`dialog_two_actions.xml`) with Delete in the destructive slot and Cancel in the primary slot | Partial | Replaced the old `fragment_edit_lorebook.xml` pop-up (owner ruling, Aug 3 2026); one activity serves edit ("Edit {name}") and create ("Create Lorebook", no trashcan). The name/description/tag fields are the pre-existing `TextInputLayout` floating-hint boxes carried over unchanged pending a separate content pass — not yet on `Widget.App.Field.*`. Do not copy this screen as a complete field reference until that pass. |
