...
| Select AI Model | `fragment_model_selector.xml` | Shared | Full-screen `UI.Material` window with `ActionBar`/`BackButton`/`Title`; View All uses `AppButton.Primary`; favorites-first and provider-wide all-models purposes share the same search/list presentation and endpoint catalog fetch | Partial | Search is still a local outlined `TextInputLayout`, and the model/favorite row layouts have not yet been audited as shared styles. |
