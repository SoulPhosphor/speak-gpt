...
| Provider Filters Panel | `activity_provider_filter_panel.xml` | Shared (close-panel header: `Title.LeftAligned` + `CloseButton`) | Sort/quantization rows use `Widget.App.Dropdown.Label`/`.Value` with anchored `ListPopupWindow`s; capability rows use the new `Widget.App.CheckOption.Row`/`.Label` (whole-line tap, deliberately no background); Reset Filters uses `AppButton.Destructive` | Shared | Same slide-in/auto-apply pull-out mechanism as Memory Filters (`ProviderFilterState` edited in place, chart re-applies on resume), but its internals deliberately do NOT copy the memory panel's local styles — those are known-wrong. |
