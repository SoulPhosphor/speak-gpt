...
`claude/phase-8-safety-continuation-a36jxx`, then
`claude/phase-9-replacement-coordinator-ebnptm`) on September 5, 2026. Each line
reports the state of the exit-gate requirement itself.

| Exit-gate requirement | Status |
|---|---|
| Unit and instrumentation tests pass for the complete crash matrix | **Substantially done, engine-only; the full process-death matrix is not.** The 9.2 archive/manifest validation is done and unit-tested: `ChatRestoreManager` requires the supported `manifest_version` and cross-checks the manifest `chats` array against the exact archive entries before any file is read, via the pure `ChatRestorePlanner.manifestDefect` (`ChatRestorePlannerTest`). The cross-check matches what `RecoveryBackupManager` produces: the chat list is required; each declared chat's history and settings files are optional; the rejections are an unsupported version, a missing chat list, a per-chat file for an undeclared chat, and a duplicate or unsafe id. The 9.2 journal-integrity items are Robolectric-tested: `writeJournal` returns the real commit result and a commit that does not land stops the restore before quarantine/swap; the journal stores each staged file's SHA-256; and startup re-verifies the staged bytes by hash before finishing a swap (`ChatRestoreJournalRecoveryTest`). The remaining 9.2 hardening is now done and tested: `performSwap` treats a delete or copy that does not take as a failure, and after the swap the engine verifies the live chat-storage set exactly matches the manifest hashes — every declared file present with its hash, no unlisted file — via the pure `ChatRestorePlanner.liveSetDefect`, before it clears the journal (`ChatRestorePlannerTest`, and an end-to-end `ChatRestoreEngineTest` that restores a real archive, replaces the complete set, deletes a superseded file, and rejects a hash-mismatched archive before any mutation). The 9.1 replacement coordinator is built (`ChatSetReplacementCoordinator`): `ChatRestoreManager` routes through it; before staging it settles the recoverable pending operations by running their existing reconcilers, then refuses with a typed reason if a rename, deletion, first-conversation commit, or provisional session is still pending or unreadable — never silently clearing a journal (pure `blockingReason` unit tests; a Robolectric settle-or-refuse test). The 9.3 dependent-store rebase is built and, after a verified swap, the coordinator stamps a new source generation, evicts the replaced preference handles, discards and rebuilds Search from the new source (so no stale/null-revision row survives), and requeues the generated-image catalog's restored chats for backfill while preserving rows, tombstones and locks (Robolectric Search-rebase and source-generation tests; the catalog requeue is an instrumented test — compiled in CI, run only on a device). The rebase is durability-gated: `onAuthoritativeChatSetReplaced` reports whether it completed, and `ChatRestoreManager` clears the restore journal (live and resume paths) only when it did — otherwise the verified swap stands but the `SWAPPING` journal and staging are kept so the resume path retries the rebase idempotently, rather than reporting a success that left a stale derived store (Search's `requiresRebuild` checks generation/policy/locale, not the restore source generation, so a Search index that survived a failed discard would otherwise never be rebuilt). Failure-injection tests cover both a Search discard that cannot delete the index and a restore whose rebase fails keeping the journal `SWAPPING`. The pre-replacement pending-operation gate is race-free: `settleOrRefuse` runs before staging, and the engine re-runs a read-only `pendingBlockNow` check INSIDE `CHAT_LIST_LOCK` immediately before quarantine, so a chat mutation that appeared between the preflight and the lock is caught and the restore refuses; provisional-conversation creation now also takes `CHAT_LIST_LOCK` (previously the one chat mutation that did not), so a provisional can never be half-created and swept while the restore holds the lock (a real concurrency test proves the creation blocks on the lock; a `pendingBlockNow` test proves a late provisional is caught). The live-path rebase is passed the manifest's full declared chat-id set, so a declared-but-empty chat's cached preference handles are invalidated too. `resumeIfPending` was moved ahead of generated-image catalog maintenance at startup. All unit and Robolectric tests execute green on `Android Checks`. **Still not done:** the fail-closed `UNRECOVERABLE` recovery state that offers verified retry/quarantine choices (awaits owner-approved wording); a full crash-matrix with process-death after every numbered 9.1 step; a terminal bound on rebase retries (a permanently un-rebasable derived store currently retries every start — non-destructive, since the live chat set is correct, but it belongs with the deferred fail-closed recovery state); the generated-image catalog requeue on a catalog that is merely locked during the restore is deferred to the post-restart maintenance rather than gated; the resume path invalidates cache from entry-derived ids only (harmless — an empty chat has no per-chat files to invalidate, and the restart is the real mechanism); and the coordinator's post-swap dependent-store work still runs inside `CHAT_LIST_LOCK` (consistent with the codebase's verified `CHAT_LIST_LOCK → SecurePrefs` order — not a demonstrated inversion — and moving it out would open a post-swap window where the chat set has changed but dependent stores have not yet been invalidated, so it stays put). |
| A source-contract test proves the engine has no reachable caller until the approved UI is implemented | **Met, engine-only.** `RestoreEngineHasNoReachableCallerTest` (unit CI) fails if any production source other than `ChatRestoreManager` reaches `restoreFromArchive`. `restoreFromArchive` has no production caller; the new `ChatSetReplacementCoordinator` is reached only from `ChatRestoreManager`, so the whole path is unreachable until the approved restore UI exists. `resumeIfPending` remains the only startup finisher and is inert while no reachable restore can leave a journal. |
