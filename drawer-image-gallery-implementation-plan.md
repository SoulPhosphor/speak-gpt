...
|---|---|
| Unit and instrumentation tests pass for the complete crash matrix | **Partial, engine-only.** The 9.2 archive/manifest validation is done and unit-tested: `ChatRestoreManager` now requires the supported `manifest_version` and cross-checks the manifest `chats` array against the exact archive entries before any file is read, via the pure `ChatRestorePlanner.manifestDefect` (`ChatRestorePlannerTest`). The cross-check matches what `RecoveryBackupManager` actually produces: the chat list is required; each declared chat's history and settings files are optional (a brand-new or empty chat — an authoritative MISSING/EMPTY read — is a legitimate chat-list row with no per-chat file, and `addEncFile` omits any file that does not exist); the rejections are an unsupported version, a missing chat list, a per-chat file for an undeclared chat, and a duplicate or unsafe id. A producer/validator agreement test covers a mixed set (both files, history-only, settings-only, neither). The 9.2 journal-integrity items are also done and Robolectric-tested: `writeJournal` now returns the real commit result and a commit that does not land stops the restore before it quarantines or swaps; the journal stores each staged file's expected SHA-256; and startup re-verifies the staged bytes by hash (`stagedBytesMatch`) before finishing a swap, so a tampered, truncated, or missing staging is refused and the pre-restore quarantine stays the recovery source (`ChatRestoreJournalRecoveryTest`). Still not started: the Phase 9.1 replacement coordinator (`ChatRestoreManager` remains the pre-Phase-9 raw-swap engine — the plan's P1 risk), the dependent-store rebase (9.3), and the remaining 9.2 hardening — delete/copy failures are not yet treated as failures with a final-active-set verification, and the fail-closed UNRECOVERABLE recovery state that offers verified retry/quarantine choices still awaits owner-approved wording. No full crash-matrix (process-death after every step) tests exist. |
